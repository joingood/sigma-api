<template>
    <div class="row container"> 
        ...
        <div class="col s12">
            <a class="waves-effect waves-light btn" href="#/hotel/create">Nuevo Registro</a>
        </div>
                ...
                <tbody  >
                    <tr v-for="(data,i) in dataTable" :key="data.Code" >
                        ...
                        <td>
                            ...
                            <router-link :disabled="process"  class="btn-floating btn-large waves-effect waves-light blue"  :to="{ path: 'hotel/update/'+ data.Id}"><i class="material-icons">update</i></router-link>
                        </td>
                    </tr>
...
export default {
    ...
    methods: {
        del(Id){
            this.process = true;
            this.$http.delete(window.__sigma.URL_REQUEST("hotel/delete"), { params: {Id},  Id})
            .then(response=>{
                //console.log( response.body );
                if(response.body.Error == 0){
                    M.toast({html: 'El hotel se elimino exitosamente!'}); //mostrando un mensaje de exito
                    setTimeout(()=>{
        ...
        search(){
            ...
            this.$http.get(window.__sigma.URL_REQUEST("hotel/list"))
            .then(response=>{
                this.process = false;
